...
const UsersListSection = ({usersList}: { usersList: User[] }) => {
    ...
    return (<>
        <form className="h-12 flex">
            <input type="email" placeholder='myfriend@example.dev' spellCheck='false' onChange={(e) => setFilter(e.target.value)} className="peer w-full text-lg text-slate-200 bg-[#1e1e1e] border-2 border-[#3c3c3c] rounded-md invalid:border-pink-500 invalid:text-pink-600 focus:outline-none px-1 placeholder:text-zinc-600"/>
        </form>
